...
class CheatCodeFixed(Policy):
    """CheatCode variant with corrected quaternion math and tighter runtime behavior."""
    ...
    def _lookup_port_transform(self, task: Task) -> Transform:
    ...
    def calc_gripper_pose(
        ...
        # Correct inverse for (w, x, y, z) unit quaternion.
        q_plug_inv = (
            q_plug[0],
            -q_plug[1],
            -q_plug[2],
    ...
    def insert_cable(
        ...
        self.get_logger().info(f"CheatCodeFixed.insert_cable() task: {task}")
        self._task = task
        ...
        # Respect task time limit to avoid forced timeout failures.
        deadline = self.time_now() + Duration(seconds=float(task.time_limit))
        z_offset = 0.2

        # Reset integrator once before approach.
        self._tip_x_error_integrator = 0.0
        self._tip_y_error_integrator = 0.0

        for t in range(0, 100):
            if self.time_now() >= deadline:
                self.get_logger().warn("CheatCodeFixed timed out during approach.")
                return False
            interp_fraction = t / 100.0
            try:
                port_transform = self._lookup_port_transform(task)
        ...
        while z_offset >= -0.015:
            if self.time_now() >= deadline:
                self.get_logger().warn("CheatCodeFixed timed out during descent.")
                return False
            z_offset -= 0.0005
            try:
                port_transform = self._lookup_port_transform(task)
        ...
        settle_time_sec = min(5.0, max(0.0, float(task.time_limit) * 0.2))
        if settle_time_sec > 0.0:
            self.get_logger().info(
                f"Waiting {settle_time_sec:.1f}s for connector stabilization..."
            )
            self.sleep_for(settle_time_sec)

        self.get_logger().info("CheatCodeFixed.insert_cable() exiting...")
        return True
